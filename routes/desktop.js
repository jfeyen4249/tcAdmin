...
router.get("/search", session.validateSession,  (req, res) => {
    ...
    database.query(`SELECT * FROM computers WHERE view = 'true' AND type = 'desktop' AND (name LIKE ? OR make LIKE ? OR model LIKE ? OR mac LIKE ? OR room LIKE ? OR tag LIKE ? OR building LIKE ? OR ip LIKE ? OR os LIKE ? OR user LIKE ?) ORDER By building ASC`,
      [
        `%${searchQuery}%`,
        `%${searchQuery}%`,
        `%${searchQuery}%`,
